...
{
    MachineTelemetry t;
    t.machineId = id; t.machineName = name; t.state = state;
    t.loadPercent = load_percent; t.temperature = temperature; t.errorCount = error_count;
    return t;
}

CNC_Machine::CNC_Machine(int id, const std::string& name) : BaseMachine(id, name)
{
    currentX = currentY = currentZ = targetX = targetY = targetZ = startX = startY = 0.0;
    spindelSpeed = feedRate = rotateRadius = arcAngle = targetAngle = centerX = centerY = 0.0;
    isCutting = false; wearing = 1.0; currentGCodeMode = 0;
}

void CNC_Machine::setTargetCoordinates(double x, double y, double z, double feed, double speed, int gMode, double radius)
{
    startX = currentX; startY = currentY; targetX = x; targetY = y; targetZ = z;
    feedRate = feed; spindelSpeed = speed; currentGCodeMode = gMode; rotateRadius = radius;
...
{
    if (state == MachineState::FAULT) { load_percent = spindelSpeed = 0.0; isCutting = false; return; }
    ...
    if (state == MachineState::WORKING)
    {
        ...
        {
            double dx = targetX - currentX; double dy = targetY - currentY; double dz = targetZ - currentZ;
            double distance = std::sqrt(dx * dx + dy * dy + dz * dz);
            if (distance > 0.0)
            {
                double step = (currentGCodeMode == 0 ? 30.0 : speedMMPerSec) * deltaTime;
                if (step >= distance) { currentX = targetX; currentY = targetY; currentZ = targetZ; }
                else { currentX += (dx / distance) * step; currentY += (dy / distance) * step; currentZ += (dz / distance) * step; }
            }
        }
        else if ((currentGCodeMode == 2 || currentGCodeMode == 3) && rotateRadius > 0.0)
        {
            ...
            if (currentGCodeMode == 2) { arcAngle -= angleStep; if (arcAngle <= targetAngle) arcAngle = targetAngle; }
            else { arcAngle += angleStep; if (arcAngle >= targetAngle) arcAngle = targetAngle; }
            currentX = centerX + rotateRadius * std::cos(arcAngle);
            currentY = centerY + rotateRadius * std::sin(arcAngle);
            if (currentZ != targetZ) currentZ += (targetZ - currentZ) * 0.1;
        }
        ...
        else { load_percent = spindelSpeed > 0 ? 6.0 : 0.0; }

        if (spindelSpeed > 0) {
            temperature += (spindelSpeed * 0.00004) * deltaTime;
            if (temperature >= FactorySettings::CRITICAL_TEMPERATURE) {
                setState(MachineState::FAULT); error_count++;
                std::cout << "\n[АВАРИЯ ПЛК] Критический нагрев шпинделя: " << temperature << "°C!\n";
            }
        }
    }
    else
    {
        load_percent = 0.0;
        if (temperature > FactorySettings::ROOM_TEMPERATURE) temperature -= 0.3 * deltaTime;
    }

    modbus_registers[FactorySettings::MB_CNC_STATUS] = static_cast<uint16_t>(state);
    modbus_registers[FactorySettings::MB_CNC_X] = static_cast<uint16_t>(currentX * 10.0);
    modbus_registers[FactorySettings::MB_CNC_Y] = static_cast<uint16_t>(currentY * 10.0);
    modbus_registers[FactorySettings::MB_CNC_Z] = static_cast<uint16_t>(currentZ * 10.0);
    modbus_registers[FactorySettings::MB_CNC_TEMP] = static_cast<uint16_t>(temperature);
    modbus_registers[FactorySettings::MB_CNC_LOAD] = static_cast<uint16_t>(load_percent);
    modbus_registers[FactorySettings::MB_CNC_WEAR] = static_cast<uint16_t>(wearing * 1000.0); 
}

void CNC_Machine::executeCuttingProgramm(std::string filePath) { setState(MachineState::WORKING); }
...
Conveyor_Machine::Conveyor_Machine(int id, const std::string& name) : BaseMachine(id, name)
{
    currentPosition = beltSpeed = 0.0; targetPosition = FactorySettings::CONVEYOR_MAX_LENGTH; itemDetected = false;
}

void Conveyor_Machine::executeMoveProgramm()
{
    if (state == MachineState::IDLE && !itemDetected) { beltSpeed = FactorySettings::CONVEYOR_DEFAULT_SPEED; currentPosition = 0.0; setState(MachineState::WORKING); }
}

void Conveyor_Machine::updatePhysics(double deltaTime)
{
    if (state == MachineState::FAULT) { beltSpeed = load_percent = 0.0; return; }
    if (state == MachineState::WORKING)
    {
        currentPosition += beltSpeed * deltaTime;
        load_percent = itemDetected ? 40.0 : 15.0;
        if (currentPosition >= targetPosition) { currentPosition = targetPosition; setSensor(true); beltSpeed = 0.0; setState(MachineState::IDLE); }
    }
    else { load_percent = beltSpeed = 0.0; }

    modbus_registers[FactorySettings::MB_CONVEYOR_POS] = static_cast<uint16_t>(currentPosition * 100.0);
    modbus_registers[FactorySettings::MB_CONVEYOR_SPEED] = static_cast<uint16_t>(beltSpeed * 10.0);
    modbus_registers[FactorySettings::MB_CONVEYOR_SENSOR] = itemDetected ? 1 : 0;
}
